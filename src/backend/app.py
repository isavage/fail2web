...
def create_jail_from_template():
    try:
        ...
        return jsonify({
            'status': 'success',
            'message': f'Jail {jail_name} created from template {template_name}',
...
"""
        ...
        ignoreip_list = []
        if 'DEFAULT' in config and 'ignoreip' in config['DEFAULT']:
            ignoreip_text = config['DEFAULT']['ignoreip']
            ignoreip_list = [ip.strip() for ip in ignoreip_text.split('\n') if ip.strip()]
        
        return jsonify({'ignoreip': ignoreip_list})
        
    except Exception as e:
        logger.error(f"Error getting ignoreIP configuration: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def update_ignoreip():
    try:
        ...
        return jsonify({
            'status': 'success',
            'message': 'ignoreIP configuration updated',
            'reload_response': reload_response
        })
